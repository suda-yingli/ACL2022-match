...
class Instance(object):
    def __init__(self, id, lines, domain_id):
        ...
        if domain_id == 2:
            self.domains_i = np.random.choice(a=domains, size=n1, replace=True, p=[0.2,0.8])#yli:19-4-29
        else:
            self.domains_i = np.random.choice(a=domains, size=n1, replace=True, p=[0.8,0.2])#yli:19-4-29
