...
class Parser(object):
    ...
    def init_models(self):
        ...
        if self._conf.is_shared_lstm:
            for i  in range(self._conf.domain_size+1):
        ...
        if self._conf.is_meta:
            self._meta_layers.append(WeightNetwork(self._source_model_list, self._pairs))
            self._meta_layers.append(LossWeightNetwork(self._source_model_list,\
                    self._pairs, self._loss_weight_type, self._loss_weight_init))

            self._feature_match_layer.append(FeatureMatching(self._source_model_list,self._target_model_list,self._pairs))
    ...
    def forward(self, words, ext_words, tags, masks, domains, domain_id, word_lens, chars_i, wordbert, unlabel=False):
        ...
        if (i == 2 and self.use_meta):
            src_out, src_out_all = self._lstm_layer[1](input_out, lstm_masks, initial=None, is_training=is_training)
            weights = self._meta_layers[0](src_out_all)
            if self._conf.is_meta_weight:
                loss_weights = self._meta_layers[1](src_out_all)
            else:
                loss_weights = None
            meta_loss_weigt = [self._conf.meta_loss] * len(self._pairs)
            matching_loss = self._feature_match_layer[0](src_out_all,pri_out_all, weights, meta_loss_weigt, loss_weights)
        if self.match_only:
            return matching_loss

        if self._conf.is_shared_lstm:
            sha_out, sha_out_all =self._lstm_layer[0](input_out, lstm_masks, initial=None, is_training=is_training)
            lstm_out = sha_out + pri_out
        else:
            lstm_out = pri_out
        ...
        if (self._conf.is_shared_lstm and self.use_meta):
            return arc_scores, label_scores, diff, matching_loss
        elif self._conf.is_shared_lstm:
            return arc_scores, label_scores, diff
        elif self.use_meta:
            return arc_scores, label_scores, matching_loss
        else:
    ...
    def train(self):
        ...
        self._eval_metrics.clear()
        current_las,all_iter =0,100
        self.set_training_mode(is_training=True)
        bc, pb = self._train_datasets[0].batch_num,self._train_datasets[1].batch_num
        #unlabel_batch_num = self._unlabel_train_datasets[0].batch_num
        bc_2pb =bc + 2*pb
        bc2_2pb =2*bc - 2*pb
        pb2 = 2*pb
        pb3 = 3*pb
        ...
        while True:
            if (train_iter < bc):
                self.use_meta,self.match_only,self.is_diff_loss =False,False,True
                print("parser src:",train_iter)
                inst_num, loss = self.train_or_eval_one_batch(self._train_datasets[0], is_training=True, unlabel=False, eval_iter = -1)
            else:
                if (bc <= train_iter <bc2_2pb):
                    self.use_meta,self.match_only =True,True
                    print("match unlabel tgt:",train_iter)
                    inst_num, loss = self.train_or_eval_one_batch(self._unlabel_train_datasets[0], is_training=True, unlabel=True, eval_iter = -1)
                elif (train_iter % 2 == 0):
                    self.use_meta,self.match_only =True,True
                    print("match unlabel tgt:",train_iter)
                    inst_num, loss = self.train_or_eval_one_batch(self._train_datasets[1], is_training=True, unlabel=False, eval_iter = -1)
                else:
                    self.use_meta,self.match_only,self.is_diff_loss =False,False,False
                    print("total label tgt:",train_iter)
                    inst_num, loss = self.train_or_eval_one_batch(self._train_datasets[1], is_training=True, unlabel=False, eval_iter = -1)
            train_iter += 1
            ...
            self.zero_grad(self._all_layers)
            if self.use_meta:
    ...
    def train_or_eval_one_batch(self, dataset, is_training, unlabel = False, eval_iter = -1):
        ...
        if unlabel == False:
            ...
            if (self.use_meta and self.match_only):
                matching_loss = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert)
            elif (self._conf.is_shared_lstm and self.use_meta):
                arc_scores, label_scores, diff, matching_loss  = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert)
            elif self._conf.is_shared_lstm:
                arc_scores, label_scores, diff = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert)
            elif self.use_meta:
                arc_scores, label_scores, matching_loss = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert)
            else:
            ...
            if self.use_meta:
                self._eval_metrics.loss_accumulated += matching_loss.item()
            if (self.use_meta and self.match_only):
                final_loss = matching_loss
            ...
            words, ext_words, lstm_masks, domains, domains_nadv, word_lens, chars_i, tags, wordbert = \
                    self.compose_batch_data_variable(one_batch, max_len, unlabel)
            if (self.use_meta and self.match_only):
                matching_loss = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert, unlabel)
            else:
                diff, matching_loss = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert, unlabel)

            #classficationd, nadv_class = self.forward(words, ext_words, tags, lstm_masks, domains, dataset.domain_id, word_lens, chars_i, wordbert, unlabel)
